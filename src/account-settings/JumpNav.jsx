...
const JumpNav = ({ intl }) => {
  ...
  return (
    <div
      className={classNames("jump-nav px-2.25", {
        "jump-nav-sm position-sticky pt-3": stickToTop,
      })}
    >
      <Scrollspy
        items={[
      ...
      {showPreferences && (
        <>
          <hr />
          <Scrollspy className="list-unstyled">
            <li>
              <Link
                to="/notifications"
                target="_blank"
              ...
              >
                <span>
                  {intl.formatMessage(
                    messages["notification.preferences.notifications.label"]
                  )}
                </span>
                <Icon
                  className="d-inline-block align-bottom ml-1"
                  src={OpenInNew}
                />
              </Link>
            </li>
